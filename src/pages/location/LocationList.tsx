...
const LocationList = () => {
  ...
  const { locationList, allotLocationLoading,loading } = useAppSelector((state) => state.location);
  ...
  const handleSubmit = () => {
    ...
    dispatch(allotLocation(payload)).then((res: any) => {
      if (res?.payload?.data?.code==200) {
        ...
        showToast(res?.payload?.data?.message?.msg, "error");
      }
    });
  };
        ...
        <div className=" h-[calc(100vh-325px)] overflow-y-auto">
          <div className="grid grid-cols-4 gap-[10px] p-[20px]">
            {locations?.map((location:any) => (
              <FormControlLabel key={location.loc_all_key} sx={{ maxHeight: "max-content" }} control={<Checkbox checked={selectedLocations.includes(location.loc_all_key)} onChange={(event) => handleLocationChange(event, location.loc_all_key)} />} label={location.module_name || "Unnamed Location"} />
            ))}
          </div>
        </div>
      )}
