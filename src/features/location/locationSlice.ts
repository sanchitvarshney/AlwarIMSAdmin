...
// Fetch all base locations for checkbox list in dialog
export const getAllLocations = createAsyncThunk<AxiosResponse<any>>(
  "location/getAllLocations",
  async () => {
    const response = await axiosInstance.get("/location/getAllLoc");
    return response;
  }
);
...
export const fetchLocationUpdate = createAsyncThunk<AxiosResponse<any>, string>(
  ...
  async (id) => {
    const response = await axiosInstance.post(
      `/location/fetch_location_all_update`,
      {
        key: id,
      }
    );
...
const locationSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      // getAllLocations -> map {location_key, loc_name} to {code, name}
